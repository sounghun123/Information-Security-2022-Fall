...
def sdes(text: bitarray, key: bitarray, mode) -> bitarray:
    ...
    if mode == MODE_ENCRYPT:
        ...
        round1 = round(ip_bits[4:8], round_key1)
        round1 ^= ip_bits[0:4]

        # XOR 적용한 값과 IP의 우측 4비트 값을 스위치해서 다음 라운드 진행
        switch_result = ip_bits[4:8] + round1

        # switch한 결과 값의 좌측 4비트 값과 F(round 함수 결과) 값 XOR 적용
        round2 = round(switch_result[4:8], round_key2)
        round2 ^= switch_result[0:4]

        # 마지막 결과 값을 IP의 역함수로 치환 후 result에 저장
        encrypt_result = round2 + switch_result[4:8]
    ...
    # Decrypt -> round_key2. round_key1
        ...
        # IP의 좌측 4비트 값과 F(round 함수 결과) 값 XOR 적용
        round1 = round(ip_bits[4:8], round_key2)
        round1 ^= ip_bits[0:4]
        ...
        # switch한 결과 값의 좌측 4비트 값과 F(round 함수 결과) 값 XOR 적용
        round2 = round(switch_result[4:8], round_key1)
        round2 ^= switch_result[0:4]

        # 마지막 결과 값을 IP의 역함수로 치환 후 result에 저장
        decrypt_result = round2 + switch_result[4:8]
