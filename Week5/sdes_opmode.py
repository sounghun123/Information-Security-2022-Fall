...
def sdes_encrypt_ecb(text: bitarray, key: bitarray):
    ...
    # 평문은 8비트의 배수의 길이 -> 8로 나누면 블록의 개수
    block_num = int(len(text) / 8)
...
def sdes_decrypt_ecb(ciphertext: bitarray, key: bitarray):
    ...
    # 암호문도 8비트의 배수의 길이 -> 8로 나누면 블록의 개수
    block_num = int(len(ciphertext) / 8)
...
def sdes_encrypt_cbc(text: bitarray, key: bitarray, iv:bitarray):
    # 암호화 결과와 xor 연산 결과 값(임시)를 담을 bitarray
    result = bitarray()
    ...
    # 각 블록마다 암호화 진행
    # xor 연산 후 암호화한 값을 다음 블록 xor 연산에 사용
    for i in range(block_num):
        if i == 0:
            first_block = iv ^ text[0:8]
            temp_result = sdes(first_block, key, MODE_ENCRYPT)
            result = temp_result
        else:
            xor_block = temp_result ^ text[i * 8:(i + 1) * 8]
            temp_result = sdes(xor_block, key, MODE_ENCRYPT)
            result += temp_result
...
def sdes_decrypt_cbc(ciphertext: bitarray, key: bitarray, iv:bitarray):
    # 복호화 결과와 xor 연산 결과 값(임시)를 담을 bitarray
    result = bitarray()
    temp_result = bitarray()
    ...
    # 각 블록마다 복호화 진행
    # 암호문 블록을 다음 블록의 xor 연산에 사용
    for i in range(block_num):
        if i == 0:
            temp_result = sdes(ciphertext[0:8], key, MODE_DECRYPT)
            result = iv ^ temp_result
        else:
            temp_result = sdes(ciphertext[i * 8:(i + 1) * 8], key, MODE_DECRYPT)
            temp_result ^= ciphertext[(i - 1) * 8:i * 8]
            result += temp_result
